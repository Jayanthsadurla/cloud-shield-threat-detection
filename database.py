...
def user_viewfiledata(fname, owner, data, email):
    ...
    _c.execute("SELECT * FROM request WHERE filename = ? AND data = ? AND owner = ? AND email = ?", 
               (fname, data, owner, email))
    ...
    _c.execute("INSERT INTO request(filename, data, owner, status, email) VALUES (?, ?, ?, 'No', ?)",
               (fname, data, owner, email))
    _conn.commit()
    _conn.close()
    return True
